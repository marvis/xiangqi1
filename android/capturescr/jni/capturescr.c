...
int main(int argc, char **argv) 
{
  ...
  if (depth == 32) //skip transparency channel
  {
	...
	for(j = 0; j < h; j++)
	{
		for (i= 0; i<w;i++)
		{
			int src_ind = j * (w+16) + i; // framebuff is wider
			int dst_ind = j * w + i;
			uint32_t pixel32 = ((uint32_t *)gr_framebuffer[0].data)[src_ind];
			// in rgb24 color max is 2^8 per channel 
			rgb24[3*dst_ind+0]   =  pixel32 & 0x000000FF; 		//Blue
			rgb24[3*dst_ind+1]   = (pixel32 & 0x0000FF00) >> 8;	//Green
			rgb24[3*dst_ind+2]   = (pixel32 & 0x00FF0000) >> 16; 	//Red
		}
	}
  }
  else
